...
/**
 * Sets up automatic closing of new tabs/pages that open during bot execution
 * Only keeps the initial pages that were opened at startup
 * This prevents navigation issues caused by errors or default browser behavior
 */
async function setupNewTabCloser(browser, email) {
  // Get initial pages when browser is first launched
  const initialPages = await browser.pages();
  
  // Store initial page objects and target IDs for tracking
  ...
  for (const page of initialPages) {
    try {
      ...
      if (target) {
        ...
        if (targetId) {
          initialTargetIds.add(targetId);
        }
      }
    } catch (e) {
      // Ignore if target not available
    }
  }
  
  console.log(`[${email}] 🔒 Tab protection enabled - will close any new tabs that open (tracking ${initialPages.length} initial pages)`);
  
  // Function to check and close new pages
  const closeNewTabs = async () => {
    try {
      const currentPages = await browser.pages();
      
      for (const page of currentPages) {
        // Check if this page is in the initial set by object reference
        let isInitialPage = initialPageSet.has(page);
        
        // Also check by target ID as backup
        if (!isInitialPage) {
          try {
            const target = page.target();
            if (target) {
              const targetId = target._targetId || target._targetInfo?.targetId;
              if (targetId && initialTargetIds.has(targetId)) {
                isInitialPage = true;
              }
            }
          } catch (e) {
            // Ignore if target check fails
          }
        }
        
        // If not an initial page, close it
        if (!isInitialPage) {
          try {
            const url = page.url();
            const urlDisplay = url ? url.substring(0, 80) : 'unknown URL';
            console.log(`[${email}] 🚫 Closing new tab: ${urlDisplay}...`);
            await page.close();
          } catch (closeError) {
            // Page might already be closed, ignore
            if (!closeError.message.includes('Target closed') && !closeError.message.includes('Session closed')) {
              console.log(`[${email}] ⚠️  Could not close new tab: ${closeError.message}`);
            }
          }
        }
      }
    } catch (error) {
      // Ignore errors during tab checking (browser might be closed)
      if (error.message && !error.message.includes('Target closed') && !error.message.includes('Session closed')) {
        console.log(`[${email}] ⚠️  Error checking tabs: ${error.message}`);
      }
    }
  };
  
  // Listen for new targets (new tabs/pages)
  browser.on('targetcreated', async (target) => {
    try {
      const page = await target.page();
      if (page) {
        // Check if this is a new page (not in initial set)
        const isNewPage = !initialPageSet.has(page);
        
        if (isNewPage) {
          // Small delay to let page load, then close it
          await delay(300);
          try {
            const url = page.url();
            const urlDisplay = url ? url.substring(0, 80) : 'unknown URL';
            console.log(`[${email}] 🚫 Detected new tab, closing: ${urlDisplay}...`);
            await page.close();
          } catch (closeError) {
            // Page might already be closed or closing, ignore
            if (!closeError.message.includes('Target closed') && !closeError.message.includes('Session closed')) {
              console.log(`[${email}] ⚠️  Could not close new tab immediately: ${closeError.message}`);
            }
          }
        }
      }
  ...
  // Also run periodic check every 2 seconds as backup
  const checkInterval = setInterval(async () => {
    try {
      await closeNewTabs();
    } catch (error) {
      // Ignore errors in periodic check (browser might be closed)
      if (error.message && !error.message.includes('Target closed') && !error.message.includes('Session closed')) {
  ...
  // Store interval ID on browser object so we can clear it if needed
  browser._tabCloserInterval = checkInterval;
  
  return { initialPages, closeNewTabs };
}
...
async function launchAccount(accountConfig, exchangeConfig) {
  ...
  try {
    ...
    // Setup automatic closing of new tabs that open during bot execution
    // This prevents navigation to new tabs while keeping the initial tabs open
    // This handles cases where errors or default browser behavior opens new tabs
    await setupNewTabCloser(browser, email);
