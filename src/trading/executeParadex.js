...
/**
 * Paradex specific trade execution logic
 */

/**
 * Set leverage for Paradex (uses setLeverage function)
 */
export async function setLeverageParadex(page, leverage, exchange) {
  console.log(`[${exchange.name}] Setting leverage using setLeverage function...`);
  const leverageResult = await setLeverage(page, leverage);
  if (!leverageResult.success) {
    console.log(`[${exchange.name}] ⚠ Failed to set leverage: ${leverageResult.error}`);
...
/**
 * Handle TP/SL for Paradex
 */
export async function handleTpSlParadex(page, exchange) {
  console.log(`[${exchange.name}] Clicking TP/SL checkbox before executing trade...`);
  ...
  if (tpSlResult.success) {
    ...
    if (fillResult.success) {
      console.log(`[${exchange.name}] ✅ TP/SL values filled successfully`);
    } else {
      console.log(`[${exchange.name}] ⚠️  Could not fill TP/SL values: ${fillResult.error || 'unknown error'}`);
      // Continue anyway - TP/SL values might not be critical
    }
  } else {
    console.log(`[${exchange.name}] ⚠️  Could not click TP/SL checkbox: ${tpSlResult.error || 'unknown error'}`);
    // Continue anyway - TP/SL checkbox might not be critical
  }
}

/**
 * Find confirm button for Paradex (standard logic)
 */
export async function findConfirmButtonParadex(page, side, exchange) {
...
export async function executeTradeParadex(
  ...
  if (setLeverageFirst && leverage) {
    await setLeverageParadex(page, leverage, exchange);
  }
  ...
  // 4. Handle TP/SL for Paradex
  await handleTpSlParadex(page, exchange);

  // 5. Find and click Confirm button
  const { confirmBtn, confirmText } = await findConfirmButtonParadex(page, side, exchange);
